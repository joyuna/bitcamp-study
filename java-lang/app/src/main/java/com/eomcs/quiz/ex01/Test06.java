...
public class Test06 {
  ...
  static int closestIntSameBit(int x) {
    // 이 메서드를 완성하시오!
    // 몇번 반복할거냐면 부호비트 빼고 31번만 반복하자. 0부터 30까지만
    for (int i = 0; i <31; i++) {
      //반복할대 마다 한 비트씩 증가하면서 최하위 두개의 비트를 추출한다.
      int bit1 = (x >> i) & 1; // i번째 이동해 맨끝에 비트를 추출한다.
      int bit2 = (x >> i + 1) & 1; 
      
      if (bit1 != bit2) {  // 1비트와 두번째 비트가 서로 다르다면 맞교환한다. 
        int mask = (1 << i) \ (1 << (i + 1)); //마스크를 씌운다.// 맞교환할 비트를 1로 지정한다.
        x^=  mask;  //<- 오른쪽 코드를 줄인 코드    x = x ^ mask; // 두비트를 바꾸는것 // 원래 값을 마스크와 XOR하여 1로 설정된 비트의 값을 뒤집는다.
        return x;
      }
    }
